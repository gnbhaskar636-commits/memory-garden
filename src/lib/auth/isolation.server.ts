 ...
 *
 * If you ever deploy several apps under one shared parent domain (e.g.
 * several `*.yourplatform.com` subdomains), they are "same-site" to each
 * other but MUTUALLY UNTRUSTED, and a `SameSite=Lax` session cookie IS sent
 * on same-site subrequests — so without this, a malicious sibling could make
 * a SCRIPTED (fetch/XHR/form-POST) request to this app's server functions
 * and ride this app's session cookie. On a normal single-domain deploy this
 * guard is just solid defense-in-depth.
 *
